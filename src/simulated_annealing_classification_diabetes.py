...
class NeuronalNetworkClassification:
...
def simulated_annealing(neural_network_class, bounds, activation_functions, train_data, train_labels, 
    ...
    for i in range(n_iterations):
        ...
        diff = candidate_eval_best - curr_eval #diff > 0 for better evaluations
...
df = pd.read_csv(pfad + 'diabetes_binary_5050split_health_indicators_BRFSS2015.csv')
...
# Now X_train_tensor, X_test_tensor, y_train_tensor, and y_test_tensor are ready for use in TensorFlow models
# Define the hyperparameter space
#activation_functions = ['relu', 'tanh', 'sigmoid', 'leaky_relu', 'elu']
#layer_ranges = range(1, 4)  # Searching between 1 and 3 layers
#node_ranges = [126, 112,86,64,46]  # Different node counts per layer
...
best_activation_fct, best_layer, best_nodes, best_eval, best_nn = simulated_annealing(NeuronalNetworkClassification, bounds, activation_functions, X_train_tensor, y_train_tensor, X_val_tensor, y_val_tensor, n_iterations, step_size, temp, input_shape = X_train.shape[1], output_shape = 2)
...
# Assuming 'model' is your trained Keras model and 'X_test', 'y_test' are your test data and labels

# Predict the probabilities and convert them to class labels
y_pred_np =best_nn.model.predict(X_test_tensor)

# Convert one-hot encoded labels to class labels for y_true_np
if y_test_tensor.ndim > 1 and y_test_tensor.shape[1] > 1:
    y_true_labels = np.argmax(y_test_tensor, axis=1)
#else:
    #y_true_labels = y_true_np

# Convert predicted probabilities to class labels for y_pred_np
if y_pred_np.ndim > 1 and y_pred_np.shape[1] > 1:
    y_pred_labels = np.argmax(y_pred_np, axis=1)
else:
    y_pred_labels = y_pred_np

# Convert class labels to Pandas DataFrames
df_true = pd.DataFrame(y_true_labels, columns=['True Labels'])
df_pred = pd.DataFrame(y_pred_labels, columns=['Predicted Labels'])

# Combine both DataFrames
df_combined = pd.concat([df_true, df_pred], axis=1)

# Generate the confusion matrix
cm = confusion_matrix(df_combined['True Labels'], df_combined['Predicted Labels'])
